"""Tests for cull.stage2.taste — warm-start, scored-path, and shape-guard behaviour."""
...
FEATURE_ROW: tuple[float, ...] = (0.5, 0.5, 0.5, 0.5, 0.5)
...
class _StubEstimator:
    ...
    def __init__(self, probability: float, n_features_in: int) -> None:
...
def _make_input(row: tuple[float, ...] = FEATURE_ROW) -> TasteScoreInput:
    """Build a TasteScoreInput with a synthetic canonical feature row."""
    return TasteScoreInput(feature_row=np.asarray(row, dtype=np.float32))
...
def _patch_with_profile(
    ...
    """Inject a fake taste profile into the module-level cache."""
...
def test_warmstart_when_profile_missing(
    ...
    result = taste.score_one(_make_input())
...
def test_warmstart_when_label_count_below_min(
    ...
    profile = taste._TasteProfile(
        estimator=_StubEstimator(PROBE_PROBABILITY, len(FEATURE_ROW)),
        label_count=1,
        version="cold",
    )
...
def test_shape_mismatch_falls_back_to_warmstart(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    """score_one warm-starts (never crashes) when the row length doesn't match the profile."""
    profile = taste._TasteProfile(
        estimator=_StubEstimator(PROBE_PROBABILITY, n_features_in=772),
        label_count=TRAINED_LABEL_COUNT,
        version=PROFILE_VERSION,
    )
    _patch_with_profile(monkeypatch, profile)
